% HumanBat_shortclip_label_and_save.m

% Last step in the audio pipeline, for every sorted event, plot it, plot it in context, 
% listen to it, label it, and save the segment with the label in the name.
...
%desired_event = 631594301;

for dd = 30:length(sorted_events)

    desired_event = sorted_events(dd);
    ...
    % Plot that segment (10s) 
    chunk = desired_chunk;  seg_start = amount_into_chunk; seg_end = amount_into_chunk+context_interval_;
    event_vec_start = desired_event-first_ttl_sample;
    event_vec_end = desired_event+context_interval_-first_ttl_sample;
    ...
    start_sample_ciholas = round((event_vec_start)/motu_Fs*ciholas_Fs);
    end_sample_ciholas = round((event_vec_end)/motu_Fs*ciholas_Fs);
    ...
    ee_spec(desired_event+1-event_vec_start) = 1;
    ...
    %% Plot 50ms clip spectrogram and raw data 

    % Plot that segment (100ms) 
    buffer = 192000/10;
    chunk = desired_chunk;  seg_start = amount_into_chunk-buffer; seg_end = amount_into_chunk+interval_-buffer;
    event_vec_start = desired_event-first_ttl_sample-buffer;
    event_vec_end = desired_event+interval_-first_ttl_sample-buffer;
    ...
    whitenedSignal_allmics_short = cell2mat(whitenedSignal_mm);
    sum_whitenedSignal_allmics_short = sum(whitenedSignal_allmics_short,2);
        
    %% Align the above_zero_events to the flight data 
    above_zero_events = (sorted_events-event_vec_start);
    above_zero_events = above_zero_events(above_zero_events>0);
    above_zero_events = above_zero_events(above_zero_events<length(sum_whitenedSignal_allmics_short)-buffer);
    above_zero_events_aligned_to_start_of_session = above_zero_events+event_vec_start;
    [above_zero_events_aligned,ttl_events_aligned] = HumanBat_align_audio_to_ciholas(Bat,batdate,logger,alignment_,above_zero_events_aligned_to_start_of_session,pk_loc_vec,first_ttl_sample,last_ttl_sample);
    ...
    start_sample_ciholas = (event_vec_start)/motu_Fs*ciholas_Fs;
    end_sample_ciholas = (event_vec_end)/motu_Fs*ciholas_Fs;
    ...
    if save_audio_and_figure == 1
        ...
        clip_name_context = strcat('/home/madeleine/mnt/server2/users/KQMS/HumanBat/audio_clips/',num2str(batdate),'_',num2str(Bat),'_10s_clip_startsample_',num2str(desired_event),'.wav');
        audiowrite(clip_name_context, sum_whitenedSignal_allmics, motu_Fs);
    
        clip_name_short = strcat('/home/madeleine/mnt/server2/users/KQMS/HumanBat/audio_clips/',num2str(batdate),'_',num2str(Bat),'_50ms_clip_timestamp_',num2str(desired_event),'.wav');
        audiowrite(clip_name_short, sum_whitenedSignal_allmics_short, motu_Fs);
        
        clip_name_context_figure = strcat('/home/madeleine/mnt/server2/users/KQMS/HumanBat/audio_clips/',num2str(batdate),'_',num2str(Bat),'_event_start_sample_',num2str(desired_event),'.svg');
        saveas(gcf, clip_name_context_figure, 'svg');
    
        clip_name_short_figure = strcat('/home/madeleine/mnt/server2/users/KQMS/HumanBat/audio_clips/',num2str(batdate),'_',num2str(Bat),'_50ms_clip_timestamp_',num2str(desired_event),'.svg');
        saveas(gcf, clip_name_short_figure, 'svg');
    end
end

%% Go to next script for listening, looking, and labeling!
